...
def write_img_to_mem(dut, img_f32):
    "write images directly to act.mem"
    image = quantize_image(img_f32)
    for tile in range(196):
        pixels = image[tile * 4: tile * 4 + 4]
        dut.u_act_mem.mem[img_base + tile].value = pack_bytes(pixels)
...
def read_layer1_output(dut):
    ...
    for tile in range(32):
        word = int(dut.u_act_mem.mem[layer1_base + tile].value)
...
async def run_program(dut):
    ...
    for cycle in range(3_000_000):
        ...
        if int(dut.store_en.value):
            four = [dut.result_data[j].value.signed_integer for j in range(4)]
            stores.append(four)
